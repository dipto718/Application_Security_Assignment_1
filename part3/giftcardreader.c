...
struct this_gift_card *gift_card_reader(FILE *input_fd) {
	...
	//if (ret_val->num_bytes != sizeof(struct this_gift_card)){
	//	printf("Invalid gift card: the number of bytes is not properly set, it is undefined\n");
	//	abort();
		//printf("g\n");
		
	//}
	//printf("c\n");
	//printf(sizeof(struct this_gift_card) + "d\n");
	...
	while (!feof(input_fd)) {
		...
		//printf("a\n");
		//printf("%i\n", ret_val->num_bytes);
		// num_bytes only is this big if there is a segmentation fault
		if(ret_val->num_bytes > 1000000000){
			printf("giftcard attributes are improperly set, should not read further\n");
			return NULL;
		}
		if(ret_val->num_bytes < 0){
			printf("number of bytes for the giftcard should not be negative, should not read further\n");
			return NULL;
		}
		//printf("i\n");
		//printf("%i\n", ret_val->num_bytes);
